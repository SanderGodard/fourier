...
# Simuulere bølger, kan justeres til å inneholde stipling for path, eller å bare vise én av n variantene i bølgen.

def main():
#	lage args for n styring??

	#x er variabel av rom bortover langs l
	#t er variabel av tid som stiger, kun i bruk ved stipling.
	...
	n_end = 10	# Det over summasjonstegnet. Ofte inf; bruker lavere tall enn inf for simulering

	marker = l/2	# Punkt langs x som skal utmerkes

	stipla_toggle = 0 # Toggle stipla linjer av eller på. Er bare kalibrert inn på n < 5 per nå.
...
#	ax.tight_layout()
	...
	print(f"Gjør {int(t_delta**(stipla_toggle)*millis*l*(n_end-n_start+1))} målinger.") # Usikker på nøyaktigheten av denne summen, vil ikke bruke tid på det.
